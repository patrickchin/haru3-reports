...
- **EAS env vars** — manual push (no native EAS integration exists):
  - macOS / Linux: [`scripts/sync-eas.sh`](../scripts/sync-eas.sh)
  - Windows:       [`scripts/sync-eas.ps1`](../scripts/sync-eas.ps1)
...
└────────┬─────────┘
         ...
         └─ scripts/sync-eas.{sh,ps1} <env> ─→ EAS env vars
```
...
| `SERVICE_ROLE_KEY`      | [supabase-deploy.yml](../.github/workflows/supabase-deploy.yml) (manual `backfill-thumbnails` job — POSTs to the `backfill-file-thumbnails` edge function as the service role). Doppler rejects the natural name `SUPABASE_SERVICE_ROLE_KEY` because the `SUPABASE_` prefix is reserved for runtime env vars Supabase injects into edge functions. |
...
3. For EAS only: trigger **Actions → Sync EAS env vars → Run workflow**
   (or run `./scripts/sync-eas.sh <env>` on macOS/Linux,
   `pwsh ./scripts/sync-eas.ps1 <env>` on Windows).
